...
const Meal = (prop) => {
    ...
    useEffect(() => {
      // getStores();
      axios.get(`http://164.90.131.16/api/meal/${id}`)
        .then( (response) =>  {
            // handle success
            // const response =
   ...
   if(isSubmitted){
    ...
    return (
        <React.Fragment>
          ...
          {/* <div className="jumbotron col-12" style={{backgroundImage: `url(http://164.90.131.16/storage/${meal.imageName}`, backgroundSize: `cover`, height: `300px` }}>
         </div>  */}
         <div className="container">
             <div className="row">
                <div className="col">
                    <img src={`http://164.90.131.16/storage/${meal.image_name}`}  style={{ width: '100%' }}/>
                </div>
                <div className="col">
